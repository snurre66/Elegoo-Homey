...
class CapabilityMapper {
  ...
  static updatePrintInfo(device, attr) {
    ...
    const totalTicks = pi.TotalTicks ?? attr.TotalTicks;
    const currentTicks = pi.CurrentTicks ?? attr.CurrentTicks;
    const remainingTime = pi.RemainingTime ?? attr.RemainingTime;

    if (totalTicks !== undefined && currentTicks !== undefined) {
      const remainingSec = Math.max(0, totalTicks - currentTicks);
      device.safeSetCapabilityValue('time_left', CapabilityMapper.formatDuration(remainingSec));
    } else if (remainingTime !== undefined) {
      device.safeSetCapabilityValue('time_left', CapabilityMapper.formatDuration(Math.max(0, remainingTime)));
    }
  ...
  /**
   * Format seconds into digital clock string HH:MM:SS.
   * @param {number} totalSeconds
   * @returns {string} e.g. "01:15:30"
   */
